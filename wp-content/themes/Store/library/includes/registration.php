...
?>
 ...
 <div class="registernchekout_m" style="width:<?php echo $box_width;?>%">
    ...
    <p> <?php _e(REGISTER_CHECKOUT_MSG);?>  </p>
     <input class="highlight_input_btn " type="button" onclick="showhide_registration();" value="Register &raquo;" name="confirm"/>
   </div>
...
<div class="sign_in_l" style="width:<?php echo $box_width;?>%" >
    ...
    <p> <a href="javascript:void(0);showhide_forgetpw();"><u><?php _e(FORGOT_PASSWORD_TEXT);?></u></a></p>
   <input class="highlight_input_btn " type="button" onclick="showhide_login();" value="<?php _e(SING_IN_TEXT);?> &raquo;" name="confirm"/>
</div>
				 ...
				 ?>
                   <div class="checkout_r" style="width:26%" >
                    ...
                    <p> <?php _e(GUEST_CHECKOUT_MSG);?>  </p>
                     <input class="highlight_input_btn " type="button" onclick="window.location.href='<?php global $General; echo $General->get_ssl_normal_url(get_option('siteurl'));?>/?page=checkout&checkout_as_guest=1'" value="<?php _e(CHECKOUT_TEXT);?> &raquo;" name="confirm"/>
                   </div>
                 <?php
				 }
				 ?>
         <div class="form_col_1 " style="clear:both">
          <div class="form login_form" id="login_form_div_id" style="display:none;" >
            ...
            <form name="loginform" id="loginform" action="<?php echo get_option( 'siteurl' ).'/?page=login'; ?>" method="post" >
              ...
              <p class="forgetmenot">
                ...
                <?php _e(REMEMBER_ME_TEXT); ?>
              </p>
             <!-- <a  href="javascript:void(0);" onclick="chk_form_login();" class="highlight_button fl login" >Sign In</a>-->
              <input class="highlight_input_btn login" type="submit" value="<?php _e(SING_IN_TEXT); ?>" onclick="return chk_form_login();"  name="submit" />
        <?php
		global $General;
		if(strstr($_SESSION['redirect_page'],'page=checkout'))
		{
...
function chk_form_login()
{
	...
	{
		alert('<?php _e(ENTER_USERNAME_JS_MSG) ?>');
		document.getElementById('user_login').focus();
		return false;
	}
	if(document.getElementById('user_pass').value=='')
	{
		alert('<?php _e(ENTER_PW_JS_MSG) ?>');
