...
<div id="wrapper">
    ...
    <div id="main_center" class="clearfix">
        <div id="content">        
<h1 class="head"><?php _e(SINGLE_CHEKCOUT_PAGE_TITLE); ?></h1>
 ...
 <form method="post" name="checkout_frm" id="checkout_frm" action="<?php echo $General->get_ssl_normal_url(get_option('siteurl')); ?>/?page=payment&paymentmethod=<?php echo $_POST['paymentmethod'];?>">
        ...
        <?php
          ...
          <?php if($_GET['msg']=='nopaymethod'){ echo "<div style='color:red'>".__(SELECT_PAYMETHOD_MSG)."</div><br>";}?>
          <?php
...
function check_user_info()
{
	...
	{
		alert('<?php _e(ENTER_NAME_JS_MSG);?>');
		document.getElementById('user_fname').focus();
		return false;
	}
	if(document.getElementById('user_email').value=='')
	{
		alert('<?php _e(ENTER_EMAIL_JS_MSG);?>');
...
function chk_form_reg()
{
	...
	?>
		...
		{
			alert('<?php _e(ENTER_ADDRESS_JS_MSG) ?>');
...
function checkemail(str)
{
	...
	{
		alert("<?php _e(ENTER_VALID_EMAIL_JS_MSG);?>")
		return false;
	}
}
</script>
