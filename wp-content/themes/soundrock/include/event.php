...
// event start
    ...
    add_filter('manage_cs_event_posts_columns', 'event_columns_add');
		function event_columns_add($columns) {
			$columns['category'] = 'Category';
			$columns['author'] = 'Author';
			return $columns;
    }
    add_action('manage_cs_event_posts_custom_column', 'event_columns');
	...
	function cs_event_register() {  
        ...
        register_post_type( 'events' , $args );  
			...
			// adding Manage Location start
				$labels = array(
					'name' => _x('Manage Location', 'post type general name'),
					'add_new_item' => __('Add New Location (Venue Title)'),
					'edit_item' => __('Edit Location'),
					'new_item' => __('New Location Item'),
					'add_new' => _x('Add New Location', 'Add Location'),
                    ...
                    <td width="40%" valign="top">
                        ...
                        <script src="<?php echo get_template_directory_uri()?>/scripts/jquery-gmaps-latlon-picker.js"></script>
                        	<table width="100%">
                              <tr>
                                <td>Address:</td>
                                <td><input name="loc_address" id="loc_address" type="text" value="<?php echo $loc_address?>" class="gllpSearchButton" onblur="gll_search_map()" /></td>
                              </tr>
                              <tr>
                                <td>City / Town:</td>
                                <td><input name="loc_city" id="loc_city" type="text" value="<?php echo $loc_city?>" class="gllpSearchButton" onblur="gll_search_map()" /></td>
                              </tr>
                              <tr>
                                <td>Post Code:</td>
                                <td><input name="loc_postcode" id="loc_postcode" type="text" value="<?php echo $loc_postcode?>" class="gllpSearchButton" onblur="gll_search_map()" /></td>
                              </tr>
                              <tr>
                                <td>Region:</td>
                                <td><input name="loc_region" id="loc_region" type="text" value="<?php echo $loc_region?>" class="gllpSearchButton" onblur="gll_search_map()" /></td>
                              </tr>
                              <tr>
                                <td>Country:</td>
                                <td>
		...
		if ( isset($_POST['event_loc_meta_form']) and $_POST['event_loc_meta_form'] == 1 ) {
			...
			function event_loc_meta_save( $post_id ) {
				$sxe = new SimpleXMLElement("<event_loc></event_loc>");
					...
					$sxe->addChild('loc_address',$_POST['loc_address']);
            ...
            <div class="option-sec" style="margin-bottom:0;">
                <div class="opt-conts">
                    ...
                    <ul class="form-elements">
                        ...
                        <li class="to-field">
                            <input type="text" name="event_booking_url" value="<?php echo $event_booking_url?>" />
                        </li>
                        <li class="to-desc">
                            <p>
                                Put booking URL
	...
	// event custom fields save start
		if ( isset($_POST['event_meta_form']) and $_POST['event_meta_form'] == 1 ) {
			...
			function cs_event_meta_save( $post_id ) {
				...
				if ( isset($_POST['num_repeat'] ) ) {
					...
					$to_date = $_POST["event_to_date"];
						for ( $i = 1; $i < $_POST['num_repeat']; $i++ ) {
							...
							events_meta_save($_POST, $inserted_id);
							if ( $_POST['repeat'] <> 0 ) {
				...
				events_meta_save($_POST, $post_id);
