...
if ( empty($_REQUEST['last']) ) {
	...
	if ( empty($_REQUEST['nameform']) ) {
		$pass = 1;
		$alert .= "<li>" . $emptyname . "</li>";
	} elseif ( preg_match( "/[{}()*+?.\\^$|]/", $_REQUEST['nameform'] ) ) {
		$pass = 1;
		$alert .= "<li>" . $alertname . "</li>";
	}
	if ( empty($_REQUEST['emailform']) ) {
		$pass = 1;
		$alert .= "<li>" . $emptyemail . "</li>";
	} elseif ( !preg_match("/^[_a-z0-9-]+(.[_a-z0-9-]+)*@[a-z0-9-]+(.[a-z0-9-]+)*(.[a-z]{2,3})$/", $_REQUEST['emailform']) ) {
		$pass = 1;
		$alert .= "<li>" . $alertemail . "</li>";
	}
	if ( empty($_REQUEST['messageform']) ) {
		$pass = 1;
		$alert .= "<li>" . $emptymessage . "</li>";
	} elseif ( preg_match( "/[][{}()*+?\\^$|]/", $_REQUEST['messageform'] ) ) {
		$pass = 1;
	...
	} elseif (isset($_REQUEST['messageform'])) {
		// If the user didn't err and there is in fact a message, time to email it.
		$message = clean_var($_REQUEST['messageform']);
		$header = 'From: ' . clean_var($_REQUEST['name']) . 'subject: ' . clean_var($_REQUEST['subjectform']) . ' <' . clean_var($_REQUEST['emailform']) . '>';
