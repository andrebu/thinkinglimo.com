...
function bold_shortcode($atts, $content = null) {
	extract(shortcode_atts(array("text" => ''),$atts));
	return '<span class="bold-text">' . do_shortcode($content) . '</span>';
}
add_shortcode("bold_text", "bold_shortcode");

// Bold Text
function light_shortcode($atts, $content = null) {
	extract(shortcode_atts(array("text" => ''),$atts));
	return '<span class="light-span-text">' . do_shortcode($content) . '</span>';
}
add_shortcode("light_text", "light_shortcode");
...
add_shortcode("blue_text", "blue_shortcode");
...
function progress2_shortcode($atts, $content = null) {
    ...
    return '<div class="call-clearfix"></div><div class="col" >
    <div class="circular-bar-green donutalign firaschart" data-percent="'. $percent .'" data-color="'. $color .'"></div>
                ...
                </div><div class="call-clearfix"></div>';
}
...
function cooler_grid1( $atts, $content = null ) {
	...
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="one columns' . $class . '">' . do_shortcode($content) . '</div>';
}
...
function cooler_grid2( $atts, $content = null ) {
	...
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="two columns' . $class . '">' . do_shortcode($content) . '</div>';
}
...
function cooler_grid3( $atts, $content = null ) {
	...
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="three columns' . $class . '">' . do_shortcode($content) . '</div>';
}
...
function cooler_grid4( $atts, $content = null ) {
	...
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="four columns' . $class . '">' . do_shortcode($content) . '</div>';
}
...
function cooler_grid5( $atts, $content = null ) {
	...
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="five columns' . $class . '">' . do_shortcode($content) . '</div>';
}
...
function cooler_grid6( $atts, $content = null ) {
	...
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="six columns' . $class . '">' . do_shortcode($content) . '</div>';
}
...
function cooler_grid7( $atts, $content = null ) {
	...
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="seven columns' . $class . '">' . do_shortcode($content) . '</div>';
}
...
function cooler_grid9( $atts, $content = null ) {
	...
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="nine columns' . $class . '">' . do_shortcode($content) . '</div>';
}
...
function cooler_grid10( $atts, $content = null ) {
	...
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="ten columns' . $class . '">' . do_shortcode($content) . '</div>';
}
...
function cooler_grid11( $atts, $content = null ) {
	...
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="eleven columns' . $class . '">' . do_shortcode($content) . '</div>';
}
...
function cooler_grid12( $atts, $content = null ) {
	...
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="twelve columns' . $class . '">' . do_shortcode($content) . '</div>';
}
add_shortcode('grid12', 'cooler_grid12');

function cooler_grid13( $atts, $content = null ) {
	extract(shortcode_atts(array('first' => false, 'last' => false),$atts));
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="thirteen columns' . $class . '">' . do_shortcode($content) . '</div>';
}
add_shortcode('grid13', 'cooler_grid13');

function cooler_grid14( $atts, $content = null ) {
	extract(shortcode_atts(array('first' => false, 'last' => false),$atts));
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="fourteen columns' . $class . '">' . do_shortcode($content) . '</div>';
}
add_shortcode('grid14', 'cooler_grid14');

function cooler_grid15( $atts, $content = null ) {
	extract(shortcode_atts(array('first' => false, 'last' => false),$atts));
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="fifteen columns' . $class . '">' . do_shortcode($content) . '</div>';
}
add_shortcode('grid15', 'cooler_grid15');

function cooler_grid16( $atts, $content = null ) {
	extract(shortcode_atts(array('first' => false, 'last' => false),$atts));
	$class = ($first) ? ' alpha' : '';
	$class .= ($last) ? ' omega' : '';
	return '<div class="sixteen columns' . $class . '">' . do_shortcode($content) . '</div>';
}
add_shortcode('grid16', 'cooler_grid16');
