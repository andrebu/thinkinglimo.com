...
require_once ( get_template_directory()  . '/functions/theme-menuwalker.php' );
...
//require_once ( get_template_directory()  . '/functions/theme-custom-css.php' );  // CUSTOM CSS LINE - ANDRE ANDRE
...
// END of theme stock functions


// Specify favicon for Dashboard
function favicon4admin() {
 echo '<link rel="Shortcut Icon" type="image/x-icon" href="http://www.thinkinglimo.com/favicon-TL-WP-UI.ico" />';
}
...
function enqueue_gf_disable() {
	wp_enqueue_script( 'gf-disable', plugins_url( '/gravity-forms-disable/gravity-forms-disable.js' ) );
}

add_action( 'wp_enqueue_scripts', 'enqueue_gf_disable' );


//ATTEMPT TO REMOVE ?ver query strings
...
function downgrade_jquery() {
        ...
        if ( !is_admin() ) :
                ...
                wp_register_script('jquery', 'http://ajax.googleapis.com/ajax/libs/jquery/1.9.2/jquery.min.js', false, '1.9.2');
        endif;
}
